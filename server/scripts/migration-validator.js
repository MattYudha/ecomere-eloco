...
class MigrationValidator {
  ...
  async validateMigration(migrationPath) {
    ...
    if (
      ...
      warnings.push(
        '🚨 CRITICAL: Dropping column and adding NOT NULL column - potential data loss!',
      );
    }
  ...
  async getTableData(tableName) {
    ...
    try {
      ...
      console.log(
        `⚠️  Could not get data for table ${tableName}: ${error.message}`,
      );
      return [];
    }
  }
...
if (require.main === module) {
  ...
  switch (command) {
    ...
    default:
      console.log(
        'Usage: node migration-validator.js [validate|backup|safe-migrate]',
      );
  }
}

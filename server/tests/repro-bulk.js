...
(async () => {
  try {
    const cat = await prisma.category.create({
      data: { name: 'repro-' + Math.random().toString(36).slice(2, 8) },
    });
    const rows = [
      {
        title: 'Repro A',
        slug: 'repro-a-' + Date.now(),
        price: 200,
        manufacturer: 'X',
        description: 'A',
        mainImage: 'a.webp',
    ...
    const batch = await prisma.$transaction(async (tx) => {
      const created = await tx.bulk_upload_batch.create({
        data: {
          status: 'PENDING',
      ...
      return await tx.bulk_upload_batch.update({
        where: { id: created.id },
        data: {
          status: finalStatus,

...
const AdminSingleOrder = () => {
  ...
  const updateOrder = async () => {
    ...
    if (
      ...
      !isValidEmailAddressFormat(order?.email)
    ) {
    ...
    try {
      ...
      if (
        ...
        toast.error(
          'An unexpected error occurred. The server may be down or sent an invalid response.',
        );
        return;
      }
      ...
      if (error && error.details && Array.isArray(error.details)) {
        ...
        error.details.forEach((detail: { field: string; message: string }) => {
          ...
          const fieldName =
            detail.field.charAt(0).toUpperCase() + detail.field.slice(1);
          toast.error(`${fieldName}: ${detail.message}`);
        });
      } else {
        // Fallback to the general error message if 'details' is not available
        toast.error(
          error.message || error.error || 'An error occurred while updating.',
        );
      }
    } finally {
      setIsUpdating(false);
  ...
  const GlassPanel = ({
    ...
    <div
      className={`backdrop-blur-xl bg-white/70 dark:bg-black/20 border border-gray-200 dark:border-white/10 rounded-2xl shadow-lg ${className}`}
    >
      {children}
    </div>
  );
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <button
              onClick={() => router.back()}
              className="flex items-center gap-2 text-gray-500 dark:text-gray-400 hover:text-purple-600 dark:hover:text-purple-400 transition-colors"
            >
              <ArrowLeft size={20} />
              Back to Orders
            </button>
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={deleteOrder}
              className="flex items-center gap-2 px-4 py-2 rounded-lg bg-red-500/10 text-red-600 dark:bg-red-500/20 dark:text-red-400 hover:bg-red-500/20 dark:hover:bg-red-500/30 font-semibold transition"
            >
              <Trash2 size={16} />
              Delete
            </button>
            ...
            <GlassPanel>
              ...
              <div className="p-6 flex flex-col gap-4">
                {orderProducts?.map((op) => (
                  <div
                    key={op.id}
                    className="flex items-center gap-4 p-2 rounded-lg hover:bg-gray-500/10"
                  >
                    <Image
                      src={
